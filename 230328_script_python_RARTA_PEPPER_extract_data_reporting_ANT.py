...
os.chdir('/Users/clementine/Documents/TRAVAIL/PEPPER/RAR/Manip_Inserm/PHASE_1/antago')

#boucle pour appliquer le script dans tous les fichiers xlsx contenus dans le dossier # same as above
mydir = Path('/Users/clementine/Documents/TRAVAIL/PEPPER/RAR/Manip_Inserm/PHASE_1/antago')
...
#Importer la date du run, la molecule TOP, le mode, le numero du run
    ...
    IFmean = df.iat[18,4]
    IFsd = df.iat[18,5]
    Ref80mean = df.iat[19,4]
    Ref80sd = df.iat[19,5]
    Coexp= df.iat[9,4]

    data_top = [[date, molecule_top, mode, run_top, basalmean, basalsd, IFmean, IFsd, Ref80mean, Ref80sd, Coexp]]
    df_id_top = pd.DataFrame(data_top, columns = ["Date", "Molecule", "Mode", "Run number", "Basal (%mean)", "Basal (%sd)", "Induction factor (mean)", "Induction factor (sd)", "Ref80 (%mean)", "Ref80 (%sd)", "Conc coexpo (M)"])
    ...
    table_columns_id =[TableColumn(field='Date'),
                       ...
                       TableColumn(field='Ref80 (%mean)', formatter = percentformat, width = 40),
                       TableColumn(field='Ref80 (%sd)', formatter = percentformat, width = 40),
                       TableColumn(field='Conc coexpo (M)', formatter = scientificformat)
                       ]
    ...
    df_id_bot = pd.DataFrame(data_bot, columns = ["Date", "Molecule", "Mode", "Run number", "Basal (%mean)", "Basal (%sd)", "Induction factor (mean)", "Induction factor (sd)", "Ref80 (%mean)", "Ref80 (%sd)", "Conc coexpo (M)"])
    ...
    #improvements to do:
    #add ref80 for ANT and ref100 for INT
